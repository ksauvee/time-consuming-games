function test(country_name, points) {
    const proposition = document.getElementById("country").value

    if (country_name.toLowerCase() === proposition.toLowerCase()) {
        points++
    }
...
function next_flag(image) {
    const id = Math.floor((Math.random() * 254) + 1)
    image.src = "flags/" + id + ".png"
    return image
}
...
function main() {
    ...
    let points = 0
    image = next_flag(image)
    submit.onclick = function() {
        nb_flags++
        points = test(countries[image.src.split(".")[0].split("/")[image.src.split(".")[0].split("/").length-1]], points)
        if (nb_flags === 5) {
